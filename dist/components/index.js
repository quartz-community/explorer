...
var explorer_inline_default = `var U=Object.hasOwnProperty;function S(u){let e=k(W(u,"index"),!0);return e.length===0?"/":e}function T(){return typeof document>"u"?"":document.body?.dataset?.basepath??""}function v(u,e){let D=e??T(),n=u.startsWith("/")?u:"/"+u;return D+n}function I(u,e){return u===e||u.endsWith("/"+e)}function W(u,e){return I(u,e)&&(u=u.slice(0,-e.length)),u}function k(u,e){return u.startsWith("/")&&(u=u.substring(1)),!e&&u.endsWith("/")&&(u=u.slice(0,-1)),u}var y=class u{constructor(e,D){this.children=[],this.slugSegments=e,this.data=D||null,this.isFolder=!1,this.fileSegmentHint=null}get displayName(){return(this.data?.title==="index"?void 0:this.data?.title)||this.fileSegmentHint||this.slugSegment||""}get slug(){let e=this.slugSegments.join("/");return this.isFolder?e+"/index":e}get slugSegment(){return this.slugSegments[this.slugSegments.length-1]||""}makeChild(e,D){let n=[...this.slugSegments,e[0]],r=new u(n,D);return this.children.push(r),r}insert(e,D){if(e.length===0)return;this.isFolder=!0;let n=e[0];if(e.length===1)n==="index"?this.data||(this.data=D):this.makeChild(e,D);else{let r=this.children.find(F=>F.slugSegment===n);r||(r=this.makeChild(e,void 0));let t=(D.filePath||D.slug||"").split("/");r.fileSegmentHint=t[t.length-e.length],r.insert(e.slice(1),D)}}add(e){this.insert(e.slug.split("/"),e)}sort(e){this.children.sort(e),this.children.forEach(D=>D.sort(e))}filter(e){this.children=this.children.filter(e),this.children.forEach(D=>D.filter(e))}map(e){e(this),this.children.forEach(D=>D.map(e))}static fromEntries(e){let D=new u([],null);return e.forEach(([,n])=>D.add(n)),D}},P=(u,e)=>!u.isFolder&&!e.isFolder||u.isFolder&&e.isFolder?u.displayName.localeCompare(e.displayName,void 0,{numeric:!0,sensitivity:"base"}):!u.isFolder&&e.isFolder?1:-1,j=u=>u.slugSegment!=="tags";function R(u,e,D,n){return D&&u.filter(D),n&&u.map(n),e&&u.sort(e),u}async function O(u){try{console.log("[Explorer] Fetching content index...");let e=await fetchData;if(console.log("[Explorer] Fetched data keys:",Object.keys(e).slice(0,5)),!e)return console.error("[Explorer] No data received"),null;let D=e.content||e,n=Object.entries(D);if(console.log("[Explorer] Entry count:",n.length),n.length===0)return console.warn("[Explorer] No content entries found"),null;let r=y.fromEntries(n);console.log("[Explorer] Trie root children:",r.children.length);let t=P,F=j,d=null;if(u)try{let E=JSON.parse(u);E.sortFn&&(t=new Function("a","b","return ("+E.sortFn+")(a, b)")),E.filterFn&&(F=new Function("node","return ("+E.filterFn+")(node)")),E.mapFn&&(d=new Function("node","("+E.mapFn+")(node)"))}catch(E){console.error("Error parsing data functions:",E)}return R(r,t,F,d)}catch(e){return console.error("Error building file trie:",e),null}}var b=0;function w(u,e,D,n,r,t=""){let F=document.getElementById("template-folder"),d=document.getElementById("template-file");if(!F||!d)return;let E=t?t+"/"+u.slugSegment:u.slugSegment,A=S(D);if(u.isFolder){let s=F.content.cloneNode(!0),C=s.querySelector(".folder-container"),p=s.querySelector(".folder-button"),m=s.querySelector(".folder-title"),l=s.querySelector(".folder-outer"),o=s.querySelector(".content");if(m&&(m.textContent=u.displayName||u.slugSegment),C&&(C.dataset.folderpath=u.slug),n==="link"&&p){let i=document.createElement("a");i.className=p.className;let f=S(u.slug);i.href=v(f||""),m?i.appendChild(m):i.textContent=u.displayName||u.slugSegment,p.replaceWith(i),p=i}let a=r[u.slug]!==void 0?r[u.slug]:!0,c=S(u.slug),B=c&&c===A.slice(0,c.length);if((!a||B)&&l&&l.classList.add("open"),u.children&&u.children.length>0&&o)for(let i of u.children)w(i,o,D,n,r,E);e.appendChild(s)}else if(u.data){let s=d.content.cloneNode(!0),C=s.querySelector("a");C&&(C.href=v(u.data.slug),C.textContent=u.displayName||u.slugSegment,u.data.slug===D&&C.classList.add("active")),e.appendChild(s)}}async function L(u){let e=++b;try{console.log("[Explorer] Nav event received, generation:",e);let D=(u.detail?.url||"").replace(/^\\/+/,""),n=document.querySelectorAll("div.explorer");console.log("[Explorer] Found",n.length,"explorers");let r={};try{JSON.parse(localStorage.getItem("fileTree")||"[]").forEach(F=>{r[F.path]=F.collapsed})}catch(t){console.error("[Explorer] Error loading saved state:",t)}for(let t of n){let F=t.querySelector(".explorer-ul");if(!F){console.warn("[Explorer] No explorer-ul found");continue}F.innerHTML='<li class="overflow-end"></li>';let d=t.dataset.dataFns,E=t.dataset.behavior||"collapse";console.log("[Explorer] Starting tree build...");let A=await O(d);if(e===b){if(console.log("[Explorer] Render generation is current, rendering tree"),console.log("[Explorer] Trie result:",A?"success":"null"),A&&A.children&&A.children.length>0){F.innerHTML='<li class="overflow-end"></li>',console.log("[Explorer] Rendering",A.children.length,"children");for(let o of A.children)w(o,F,D,E,r,"");console.log("[Explorer] Render complete, final list length:",F.children.length)}else console.warn("[Explorer] No trie or empty children");let l=sessionStorage.getItem("explorerScrollTop");if(l)F.scrollTop=parseInt(l,10);else{let o=F.querySelector(".active");o&&o.scrollIntoView({behavior:"smooth"})}}else console.log("[Explorer] Stale render generation, skipping tree render");let s=[],C=t.getElementsByClassName("explorer-toggle");for(let l of C){let o=function(){let a=this.closest(".explorer");if(!a)return;let c=a.classList.toggle("collapsed");a.setAttribute("aria-expanded",c?"false":"true"),c?document.documentElement.classList.remove("mobile-no-scroll"):document.documentElement.classList.add("mobile-no-scroll")};l.addEventListener("click",o),s.push(()=>l.removeEventListener("click",o))}let p=t.getElementsByClassName("folder-icon");for(let l of p){let o=function(a){a.stopPropagation();let c=this.parentElement;if(!c)return;let B=c.nextElementSibling;if(!B)return;B.classList.toggle("open");let i=!B.classList.contains("open"),f=c.dataset.folderpath,h=JSON.parse(localStorage.getItem("fileTree")||"[]"),g=h.findIndex(x=>x.path===f);g>=0?h[g].collapsed=i:h.push({path:f,collapsed:i}),localStorage.setItem("fileTree",JSON.stringify(h))};l.addEventListener("click",o),s.push(()=>l.removeEventListener("click",o))}let m=t.getElementsByClassName("folder-button");for(let l of m){let o=function(a){let c=this.closest(".folder-container");if(!c)return;let B=t.dataset.behavior||"collapse",i=c.nextElementSibling,f=c.dataset.folderpath;if(B!=="link"){if(a.stopPropagation(),!i)return;i.classList.toggle("open");let h=!i.classList.contains("open"),g=JSON.parse(localStorage.getItem("fileTree")||"[]"),x=g.findIndex(N=>N.path===f);x>=0?g[x].collapsed=h:g.push({path:f,collapsed:h}),localStorage.setItem("fileTree",JSON.stringify(g))}};l.addEventListener("click",o),s.push(()=>l.removeEventListener("click",o))}typeof window<"u"&&window.addCleanup&&window.addCleanup(()=>s.forEach(l=>l()))}for(let t of document.getElementsByClassName("explorer")){let F=t.querySelector(".mobile-explorer");F&&(F.classList.remove("hide-until-loaded"),F.checkVisibility&&F.checkVisibility()&&(t.classList.add("collapsed"),t.setAttribute("aria-expanded","false"),document.documentElement.classList.remove("mobile-no-scroll")))}}catch(D){console.error("[Explorer] Fatal error in nav handler:",D)}}document.addEventListener("nav",L);document.addEventListener("render",L);document.addEventListener("prenav",()=>{let u=document.querySelector(".explorer-ul");u&&sessionStorage.setItem("explorerScrollTop",u.scrollTop.toString())});
`;
var defaultOptions = {
